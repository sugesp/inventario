...
export class LevantamentosComponent implements OnInit, OnDestroy {
  ...
  private async detectCodeFromVideoFrame(video: HTMLVideoElement): Promise<string | null> {
    ...
    const scale = Math.min(1, LevantamentosComponent.QR_SCAN_MAX_DIMENSION / Math.max(crop.width, crop.height));
  ...
  private getScannerCrop(video: HTMLVideoElement): { x: number; y: number; width: number; height: number } {
    const width = Math.round(video.videoWidth * LevantamentosComponent.QR_SCAN_CROP_WIDTH_RATIO);
    const height = Math.round(video.videoHeight * LevantamentosComponent.QR_SCAN_CROP_HEIGHT_RATIO);
