...
export class ItensInventariadosComponent implements OnInit, OnDestroy {
  ...
  async capturePhoto(): Promise<void> {
    ...
    try {
      ...
      const scale = Math.min(1, ItensInventariadosComponent.PHOTO_CAPTURE_MAX_DIMENSION / Math.max(sourceWidth, sourceHeight));
      ...
      const blob = await this.canvasToBlob(canvas, 'image/jpeg', ItensInventariadosComponent.UPLOAD_IMAGE_QUALITY);
      if (!blob) {
  ...
  private async assignStepPhoto(currentPhoto: SelectedPhoto | null, event: Event): Promise<SelectedPhoto | null> {
    ...
    try {
      const optimizedFile = await this.optimizeImageFile(
        file,
        ItensInventariadosComponent.UPLOAD_IMAGE_MAX_DIMENSION,
        ItensInventariadosComponent.UPLOAD_IMAGE_QUALITY
      );
  ...
  private async detectCodeFromImageFile(file: File): Promise<string | null> {
    const canvas = await this.renderImageToCanvas(file, ItensInventariadosComponent.QR_IMAGE_MAX_DIMENSION);
    const detector = await this.createDetector();
    if (detector) {
  ...
  persistState(): void {
    ...
    window.sessionStorage.setItem(ItensInventariadosComponent.STORAGE_KEY, JSON.stringify(state));
  }
  ...
  private restoreState(): void {
    ...
    const raw = window.sessionStorage.getItem(ItensInventariadosComponent.STORAGE_KEY);
    if (!raw) {
      return;
    }
    ...
    try {
      ...
      window.sessionStorage.removeItem(ItensInventariadosComponent.STORAGE_KEY);
    }
  }
}
